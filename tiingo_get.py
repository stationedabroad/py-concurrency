...
def _get_eod_price(stock):
    ...
    print(f'Completed for ticker: {stock} with status: {status}')


async def get_eod_price_async(stock, caller_id):
    ...
    try:
        url = url.format(ticker=stock, token=token)
        st = time()
        content = await requests.get(url, headers=header)
        status = content.status_code
        logger.info(f'Tiingo APi call for {stock} completed in: {time() - st}')
    except Exception as e:
        status = None
